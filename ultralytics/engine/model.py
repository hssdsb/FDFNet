...
from ultralytics.utils import (
    ...
    emojis,
    yaml_load,
    IterableSimpleNamespace,  # 为了在不创建trainer的情况下，将cfg转为IterableSimpleNamespace类型，用于计算损失时的损失比例读取
)
...
class Model(nn.Module):
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        add_cfg = {**DEFAULT_CFG_DICT, **self.overrides}
        self.model.args = IterableSimpleNamespace(**add_cfg)
    ...
    def _smart_load(self, key: str):
        ...
        """
        try:
            return self.task_map[self.task][key]  # key:model  self.task:detect
