...
class BaseModel(nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        csd = model.float().state_dict()  # checkpoint state_dict as FP32  将模型转为float32，确保权重以这种格式加载，后提取模型的权重字典，即所有参数OrderedDict:595
        csd = intersect_dicts(csd, self.state_dict())  # intersect 从两个OrderedDict类型的字典中返回具有相同键和相同形状的参数的交集，并排除某些指定的键
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    nc, act, scales = (d.get(x) for x in ("nc", "activation", "scales"))  # 2 none {'l': [1.0, 1.0, 512]}
    ...
    if scales:
        ...
        depth, width, max_channels = scales[scale]  # 1 1 512
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            if m in {Conv_Add, ConvTranspose_Add}:
                m.large_lr = True
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            args = [num_of_in]
            m.large_lr = True
        elif m in {Maxpool2_Add}:
            c2 = ch[f]
            m.large_lr = True
        elif m is W_Add_Tanh:
            c2 = ch[f[0]]
            num_of_in = len(f)
            args = [num_of_in, args[0]]
            m.large_lr = True
        else:
            c2 = ch[f]
